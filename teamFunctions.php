...
function showTable($mysqli, $usertable) {
	...
	echo '<input type="hidden" id="hid" name="hid" value="">
            ...
            <input type="submit" name="InsertAteam" value="Add an Entry" 
...
function populateTable($mysqli, $usertable) {
    if ($result = $mysqli->query("SELECT id, first_name, last_name, position, year FROM $usertable")) {
            ...
            //. "search_field FROM team LEFT JOIN persons ON team.persons_id=persons.id")) {
...
function ShowteamUpdateForm($mysqli, $Table) {
    ...
    if ($result = $mysqli->query("SELECT id, first_name, last_name, position, year FROM $Table WHERE id = $index")) {
        ...
        <form name="basic" method="POST" action="team.php" 
        ...
        color: white; background-color:#333333;"> <h2>Update A team</h2></td></tr>';
	...
	<tr><td>year: </td><td><input type="edit" name="year" value="' . $row[4] . '" size="20"></td></tr>';
        ...
        <tr><td><input type="submit" name="teamExecuteUpdate" class="btn btn-primary" value="Update Entry"></td> 
	</table> <input type="hidden" name="uid" value="' . $row[0] . '"> </form>
        <form action="team.php"> <input type="submit" name="BackToteam" value="Back to team" class="btn btn-primary""> </form> <br> </div>';
		}
	 $result->close();
    }
}
...
function showteamInsertForm() {
    echo '<div class="col-md-4">
        ...
        border-radius: 5px; box-shadow: 2px 2px 10px;">
        <tr><td colspan="2" style="text-align: center; border-radius: 5px;
        color: white; background-color:#333333;"> <h2>Insert New team</h2></td></tr>';
    ...
    echo '<tr><td><input type="submit" name="teamExecuteInsert"
        class="btn btn-success" value="Add Entry"></td>
        <td style="text-align: right;"> </table><a href="team.php"
